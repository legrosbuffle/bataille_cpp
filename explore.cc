...
void Exhaustive(std::span<Card> cards, GameArena& arena, std::ostream& os) {
...
void Random(std::span<Card> cards, GameArena& arena, const unsigned seed,
            std::ostream& os) {
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::cerr << argv[0] << " exhaustive|random C V [seed]\n";
    return 1;
  }
  ...
  const Deck deck = {.colors = static_cast<unsigned>(std::atoi(argv[2])),
                     .values = static_cast<unsigned>(std::atoi(argv[3]))};
  const unsigned seed =
      exhaustive ? 0
                 : (argc >= 5 ? std::atoi(argv[4]) : std::random_device()());
  ...
  if (exhaustive) {
    Exhaustive(cards, arena, os);
  } else {
    os << "seed=" << seed << "\n";
    Random(cards, arena, seed, os);
  }
  return 0;
}